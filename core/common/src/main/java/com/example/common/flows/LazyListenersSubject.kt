...
import com.example.common.flows.Empty
...
typealias ValueListener<T> = (ResultState<T>) -> Unit
...
class LazyListenersSubject<A : Any, T : Any>(
    ...
    private fun publishIfNotCancelled(record: FutureRecord<T>, argument: A, resultState: ResultState<T>) {
        if (record.cancelled) return
        publish(argument, resultState)
    }

    private fun publish(argument: A, resultState: ResultState<T>) = handlerExecutor.execute {
        futures.filter { it.key == argument }.forEach {
            it.value.lastValue = resultState
        }
        listeners.filter { it.arg == argument }.forEach {
            it.listener.invoke(resultState)
        }
    }
    ...
    private class FutureRecord<T>(
        var future: Future<*>?,
        var lastValue: ResultState<T> = Empty(),
